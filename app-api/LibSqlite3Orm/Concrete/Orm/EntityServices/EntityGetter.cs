...
public class EntityGetter : IEntityGetter
{
    ...
    private TDetails GetDetails<TTable, TDetails>(TTable record) where TDetails : new()
    {
        ...
        var masterTable = context.Schema.Tables.Values.SingleOrDefault(x => x.ModelTypeName == tableType.AssemblyQualifiedName);
        ...
        if (detailTable is not null && masterTable is not null)
        {
            ...
            {
                var fks = masterTable.ForeignKeys.Where(x => x.ForeignTableModelTypeName == detailTableType.AssemblyQualifiedName)
                    .ToArray();
